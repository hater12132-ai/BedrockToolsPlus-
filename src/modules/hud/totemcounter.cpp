...
constexpr std::string_view ItemIdentifier = "totem";
...
TotemCounterModule::TotemCounterModule()
...
void TotemCounterModule::onDisable() {
    m_count.store(0, std::memory_order_release);
}

void TotemCounterModule::onFrame() {
    if (!enabled) return;
    ...
    const std::string text = "Totems: " + std::to_string(count);
...
void TotemCounterModule::loadConfig(const nlohmann::json& json) {
...
void TotemCounterModule::saveConfig(nlohmann::json& json) {
...
void TotemCounterModule::onMenuRegistered() {
